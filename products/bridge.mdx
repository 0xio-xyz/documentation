---
title: '0xio Bridge'
description: 'Overview of the 0xio Bridge, an intent-based cross-chain protocol connecting Octra with EVM chains and Solana using FHE-encrypted intents.'
icon: 'bridge'
---

<Note>
  **Status: In Development**
  The 0xio Bridge is currently in active development. Features described represent the planned functionality and may change before production release.
</Note>
...
The **Indexer** is the central nervous system of the 0xio Bridge. It acts as the **State Coordinator** for the network.

It does not hold funds or execute transactions. Instead, it acts as a real-time scoreboard, ensuring that:
1.  **Users** know the exact status of their funds (Pending, Processing, Completed).
2.  **Solvers** know exactly which intents are waiting to be filled.
...
*   **OCT Pricing:** The price of OCT floats freely based on the **AMM Liquidity Curve** (Supply & Demand) against this unified USD pool.
*   **Benefit:** This creates a deeper liquidity pool, meaning large transfers suffer less "slippage".

### 2. Stablecoin Peg Assurance (Pyth Network)
To prevent stablecoin de-pegging from affecting the bridge's internal value, the Oracle integrates with **Pyth Network** for external verification.
*   The Oracle monitors the real-time market price of **USDC** and **USDT**.
*   This ensures that the "Virtual USD" side of the pool is always anchored to true market value, protecting the protocol from bad assets.
