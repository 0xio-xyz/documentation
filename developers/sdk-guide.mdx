...
**Package Details:**
- **Version:** 2.7.0
- **Min Extension (Mainnet):** v2.4.0+
- **Min Extension (Devnet):** v2.4.0+ (contract calls, privacy features)
- **License:** MIT
...
async function initWallet() {
  ...
  // 2. Initialize (checks for 0xio extension or any RFC-O-1 wallet)
  const isAvailable = await wallet.initialize();

  if (!isAvailable) {
    console.log("No compatible wallet detected. Install 0xio or any RFC-O-1 wallet.");
    return;
  }
...
console.log('Min Extension (Mainnet):', MIN_EXTENSION_VERSION); // "2.4.0"
console.log('Min Extension (Devnet):', MIN_EXTENSION_VERSION_DEVNET); // "2.4.0"
console.log('Supported:', SUPPORTED_EXTENSION_VERSIONS); // "^2.4.0"
