...
- **RFC-O-1 support:** Extension v2.4.0+ (for `window.octra` provider)
- **License:** MIT
...
// Connect — user approves in wallet popup
...
- **Response binding** — only responses matching a pending request ID are accepted. Forged responses are rejected.
- **No retry on rejection** — user-rejected transactions do not trigger automatic retries.
- **Configurable trusted origins** — use `setTrustedOrigins()` if your dApp runs inside a custom frame.
...
Starting in v2.4.4, the SDK supports cross-origin iframe communication for localhost development. This allows DApps running on `localhost` or `127.0.0.1` during development to communicate with the wallet through an iframe bridge, without requiring the extension content script to be injected into the development server origin. Use `setTrustedOrigins()` to allowlist your local dev server if needed.

## RFC-O-1 Provider Standard (v2.7.0)

Starting in v2.7.0, the SDK includes an `OctraProviderAdapter` that enables any RFC-O-1 compliant wallet (not just 0xio) to work with the SDK. This allows third-party wallets that implement `window.octra` to be used as a drop-in transport.
...
No code changes needed — the SDK auto-detects:
...
| 4200 | Unsupported method | `NETWORK_ERROR` |
...
Silently switch the extension's active network. No popup, no user confirmation — works like Rabby's `wallet_switchEthereumChain`.
...
const result = await wallet.sendPrivateTransfer({
  to: 'oct1x...',
  amount: 20,
  message: 'Encrypted Payroll'
});
console.log('TX Hash:', result.txHash);
```

<Note>
**Requires Extension v2.4.0+.** The extension generates FHE range proofs (64 circuits) which takes 17-300s depending on the execution engine (Desktop native → WASM rayon → WASM sequential).
</Note>
...
Moves funds from Public → Private balance (shielding). The extension generates the FHE cipher, bound proof, and submits the transaction.
...
Moves funds from Private → Public balance (unshielding). Includes range proof generation (17-300s depending on engine).
...
<Note>
Decrypt operations involve 64-circuit range proof generation. The SDK uses a 180s timeout for these calls.
</Note>
...
console.log('SDK Version:', SDK_VERSION); // "2.7.0"
