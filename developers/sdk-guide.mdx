...
**Package Details:**
- **Version:** 2.4.4
- **Min Extension (Mainnet):** v2.2.7+
- **Min Extension (Devnet):** v2.2.7+ (contract calls, privacy features)
- **License:** MIT
...
Starting in v2.4.4, the SDK automatically detects when your DApp is running inside **0xio Desktop** or **0xio App** and uses the appropriate bridge transport. No code changes are required.
...
<Warning>
**Not yet available via SDK.** Privacy operations must be performed from within the 0xio Wallet extension directly. This method returns `NOT_AVAILABLE`. DApp-initiated privacy operations are planned for a future release with `@0xio/pvac` integration.
</Warning>

Executes an FHE-encrypted private transfer (future).
...
console.log('SDK Version:', SDK_VERSION); // "2.4.4"
console.log('Min Extension (Mainnet):', MIN_EXTENSION_VERSION); // "2.2.7"
console.log('Min Extension (Devnet):', MIN_EXTENSION_VERSION_DEVNET); // "2.2.7"
console.log('Supported:', SUPPORTED_EXTENSION_VERSIONS); // "^2.2.7"
