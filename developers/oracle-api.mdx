...
- `aggregated(coingecko+dexscreener)` — Multi-source median with anomaly detection
- `pyth` — Pyth Network oracle feed (stablecoins, SOL, ETH)
...
```bash
curl -H "x-api-key: YOUR_KEY" \
  "https://oracle.0xio.xyz/api/chart/history?pair=OCT"
```
