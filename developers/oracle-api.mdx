---
title: "Oracle API"
description: "REST API reference for the 0xio Oracle with real-time price feeds, AMM swap quotes, and historical chart data for the Octra ecosystem."
icon: "chart-line"
---

The **0xio Oracle** provides real-time price feeds, swap quotes, and historical price data for the 0xio ecosystem. Prices are aggregated from multiple sources (CoinGecko + DexScreener) with anomaly detection and stored in PostgreSQL for historical chart data.
...
| `GET /api/price/oct/details` | Yes |
| `GET /api/price/twap` | Yes |
...
```json
{
  "symbol": "OCT",
  "price": 0.024816,
  "source": "aggregated(coingecko+dexscreener)"
}
```

The `source` field indicates where the price came from:
- `aggregated(coingecko+dexscreener)` — Multi-source aggregation with anomaly detection (OCT / wOCT)
- `pyth` — Pyth Network oracle feed (SOL, ETH, and other non-OCT symbols)
- `static` — Hardcoded value (stablecoins USDC / USDT)
...
Get current prices for all supported tokens (OCT, WOCT, USDC, USDT, SOL, ETH).
...
### Get OCT Detailed Market Data

```http
GET /api/price/oct/details
```

Get full aggregated market data for OCT including 24h change, volume, liquidity, and source breakdown.

**Example:**
```bash
curl -H "x-api-key: YOUR_KEY" \
  https://oracle.0xio.xyz/api/price/oct/details
```
...
```json
{
  ...
  "priceChange24h": -2.15,
  "volume24h": 12500.00,
  "liquidityUsd": 85000.00,
  "source": "aggregated(coingecko+dexscreener)",
  "confidence": "high",
  "spreadPct": 0.8,
  "allSources": {
    "coingecko": 0.024800,
    "dexscreener": 0.024832
  },
  "ethUsd": 1780.50,
  "timestamp": 1745312400000
}
```
...
Get 5-minute time-weighted average prices for OCT and ETH alongside the current spot, plus the OCT spot-vs-TWAP deviation.

**Example:**
```bash
curl -H "x-api-key: YOUR_KEY" \
  https://oracle.0xio.xyz/api/price/twap
```
...
Get historical price data over a configurable time range. Points are downsampled to fit the range (e.g. 30d → ~1 point per 2h).

**Query Parameters:**
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `pair` | string | `OCT` | Token symbol |
| `range` | string | `1d` | Time window: `1h`, `1d`, `7d` (`1w`), `30d` (`1m`), `1y` (`365d`) |

**Example:**
```bash
curl -H "x-api-key: YOUR_KEY" \
  "https://oracle.0xio.xyz/api/chart/history?pair=OCT&range=30d"
```

**Response:**
```json
[
  { "value": 1.24, "time": 1706270400 },
  { "value": 1.25, "time": 1706274000 },
  { "value": 1.26, "time": 1706277600 }
]
```

<Note>
The `time` field on chart data points is a Unix timestamp in **seconds** (suitable for charting libraries like Lightweight Charts). Other endpoints (`/health`, `/api/price/oct/details`, `/api/reserves`) return timestamps in **milliseconds**.
</Note>
...
|-------------|-------------|
| 400 | Bad Request — invalid parameters or signature payload |
| 401 | Unauthorized — invalid or missing API key / signature |
| 403 | Forbidden — address not whitelisted for key creation |
| 404 | Not Found — token or resource not found |
| 429 | Too Many Requests — rate limited (response includes `retry_after` seconds) |
| 500 | Internal Server Error |
| 502 | Bad Gateway — upstream price source error |
| 503 | Service Unavailable — oracle data not yet initialized |

Error responses use the shape `{ "error": "message" }`, with a `retry_after` field added on `429`.
